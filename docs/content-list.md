...
- **Page 1**: Up to 4 top-level items — format detail + Pokemon usage + battles fetched in parallel via `getFormatDetail(formatId, topPokemonCount=HOME_CHIP_SECTION_COUNT, lookback=selectedLookback)` + `getPokemonUsage(formatId, selectedLookback)` + `getBestPreviousDay(formatId)` (server-cached endpoint, returns flat list without pagination). The two stat sources use the window chosen in the Home `LookbackSelector` (defaults to `Week`); `getBestPreviousDay` is daily and ignores the lookback. `HOME_CHIP_SECTION_COUNT` is 10 — it mirrors the fixed row count the `/pokemon/usage` endpoint returns, so the three chip rows stay visually symmetric. Format-detail and usage errors are silently swallowed; the page still shows battles. Pagination (`hasNext`) is inferred from result size (>= 10 implies more pages). `currentPage` is set to `battlesCount / 10` so that page 2+ pagination aligns with `searchMatches`'s default limit of 10 (e.g., 50 battles → `currentPage=5`, next page requests page 6). Deduplication in `paginate()` handles any overlap when the count is not evenly divisible.
  ...
  3. `SectionGroup([...], fillWidth = true)` — built by `buildHomeStatGroup(formatDetail, usage)`; wraps up to three `StatChipRow` sections so desktop web lays them out as a responsive multi-column row that **fills the battle-card row width** (`fillWidth = true`), spreading ~3-4 chips per row per section and aligning to the same dynamic horizontal margins as "Today's Top Battles"; other platforms flatten via `unwrapSectionGroups()` and stack them vertically (chips scroll horizontally as a `LazyRow`). Each inner section is omitted when its data is empty; the whole group is omitted if all three are empty. All chips carry a `pokemonId` and navigate to that Pokemon's page on tap.
     ...
     - `Section("Trending Up", [StatChipRow(...)])` — Pokemon whose usage rose most over the window (`usage.increased`); chip subtitle is the signed change, e.g. `+5.09%`. Title at `HOME_TRENDING_UP_SECTION`. Entries with `usageChangePercent == 0.0` are filtered out; if nothing remains after the filter the section is omitted.
     - `Section("Trending Down", [StatChipRow(...)])` — Pokemon whose usage fell most over the window (`usage.decreased`); chip subtitle e.g. `-6.74%`. Title at `HOME_TRENDING_DOWN_SECTION`. Same 0% filter + omit-if-empty rule as Trending Up.
...
- **Nested navigation**: `WebApp.kt` owns a `usageNestedStack: List<NavEntry>` parallel to `desktopNavStack` (Pokemon/Player drill-downs from inside the right pane). Browser back pops `usageNestedStack` first (before `desktopNavStack`), then `searchOverlayParams`. Battle detail clicks inside the right pane are handled by the nested page's own inline pane.
- **Rankings pane collapses while a battle detail is open**: all three right-pane `ContentListPage` call sites (base Pokemon page, nested `NavEntry.Pokemon`, nested `NavEntry.Player`) pass `onBattleDetailOpenChanged` **and** `settledWidthWhenBattleOpen` (the full content-area width from a host-level `BoxWithConstraints` — see "Web Desktop: Multi-Column Battle Grid" below for why), and the left column + `VerticalDivider` are wrapped in `CollapsibleSidePane` (same component and animation as the search filter pane — see `docs/search.md`). Opening a battle at any nested depth slides the rankings pane offscreen-left, yielding `[Pokemon page | Battle detail]` instead of three cramped panes. The pane returns whenever the visible right-pane page has no open battle — e.g. drilling into a Pokemon from an open battle detail pushes a battle-less page (pane slides back in). On re-entry (tab switch back, browser back), `battleDetailOpen` is seeded via `ViewModelStore.peek()` from the cached VM of the page about to render — the nested-stack top's key at composition entry, or the base Pokemon page's key (`pokemonContentListKey`) once auto-selection resolves in the selection `LaunchedEffect` — so a restored `savedBattleId` composes the rankings pane already hidden (and the battle pane already open, via `detailPaneState` seeding in `ContentListPage`) instead of re-animating.
...
On desktop web (expanded size class), the content list uses `LazyVerticalGrid` with `GridCells.FixedSize(battleCardCellWidth)` instead of `LazyColumn`. `battleCardCellWidth` is derived from the total window width at discrete column-count breakpoints via `computeBattleCardCellWidth()` — cards grow within `[BATTLE_CARD_MIN_WIDTH, BATTLE_CARD_GROWN_MAX_WIDTH]` (560–780dp) to fill the available space, with column count stepping up at each `BATTLE_CARD_DEFAULT_WIDTH` (620dp) boundary so each column has at least the designed card width. The *target* value is keyed on a **settled layout-basis width** rather than the grid-box width, so it never recomputes mid-animation; the rendered cell width is an `Animatable` easing toward that target (`DETAIL_PANE_ANIM_DURATION_MS` + `FastOutSlowInEasing`) — but only when the target change came from a pane open/close flip; any other target change is a window resize and snaps instantly so cards track a drag. Cards therefore *scale* between their pane-open and pane-closed sizes instead of resizing in one frame — most visible on close, where the basis flips back only after the pane's exit finishes and an instant resize would land in isolation. All other geometry (pane width, grid width, Top-Pokémon tile sizing) stays keyed on the un-animated target so panes and the grid box still snap to settled positions. For standalone hosts (Home, Favorites) the basis is the page's own `BoxWithConstraints.maxWidth`, which is stable for them in both pane states. For hosts that place the page beside a `CollapsibleSidePane` (Usage rankings, Search filters), `maxWidth` itself animates frame-by-frame while that pane collapses — so those hosts pass `settledWidthWhenBattleOpen` (their full content-area width, measured by a host-level `BoxWithConstraints`), and the page uses it as the basis whenever the battle pane is open **or animating** (`detailPaneState.targetState || currentState`). Without this, opening a battle computed the grid against the pre-collapse width and then re-computed it as the side pane fed width back, producing a double reflow (e.g. 3 cols → 1 col → 2 cols). Narrow viewports (below the 2-column breakpoint) fall back to `BATTLE_CARD_DEFAULT_WIDTH` so compact mobile layouts are unchanged. This provides:
...
**Snap-to-narrow grid width**: While the battle pane is open or animating (`paneOpenOrAnimating`), the grid's outer Box modifier swaps from `Modifier.weight(1f)` to `Modifier.width(gridWidthWhenPaneOpen)`. On open this happens in the same composition as the `selectedBattleId` change, so the grid remeasures into its final (narrow) layout in a single frame. This is critical because `LazyVerticalGrid.scrollToItem(N)` in a multi-column grid gets clobbered back to the row-start by the next measure pass (`LazyGridScrollPosition.updateFromMeasureResult`). By ensuring the grid is already in its final layout before `scrollToItem` runs, the target index sticks. On close, keying on `paneOpenOrAnimating` (not `selectedBattleId`) keeps the grid frozen at its pane-open geometry until the pane's exit animation finishes, then it reflows once into the reclaimed space.

The pane width is sized dynamically from the layout-basis width: `snapDetailPaneWidth(basis - targetBattleCardCellWidth - 1.dp)` snaps the natural pane width down to the largest `PANE_SNAP_WIDTHS` target (324/596/868dp — each exactly fits N columns of open-teamsheet cards in `PlayerTeamSection`) so the pane never has empty horizontal gutters; the leftover width returns to the grid, and `grid + 1dp divider + pane = basis` exactly. `computeBattleCardCellWidth` caps card growth so the pane retains at least `DETAIL_PANEL_PREFERRED_MIN_WIDTH` (868dp) when the viewport allows it — keeping 3-pokemon-per-row in each team card. The cap floor is pinned at the default width (620dp) so cards never drop *below* the default just to buy pane width, avoiding a visible card-width jolt when resizing through the threshold where the cap first becomes relevant.

**Pane animation**: The pane uses `slideInHorizontally`/`slideOutHorizontally` (not `expandHorizontally`/`shrinkHorizontally`) so the pane's layout bounds snap to their final width the moment `visibleState` becomes true, and the content translates in from off-screen-right. This avoids an artifact where the pane's growing right edge would leave a visible sliver of the outer `surface` background to its right. The divider + panel Row inside the `AnimatedVisibility` is measured with `wrapContentWidth(align = Start, unbounded = true)`: while a host-side `CollapsibleSidePane` is still collapsing (or re-expanding on close), the outer Row offers this slot less than `panePostWidth` (the grid already holds its final width), and a plain `width()` would clamp the panel — its `PlayerTeamSection` would compose at a narrower column count and visibly reflow (2×2 → 3×3) once the side pane settled. Unbounded measurement lets the panel lay out at its full settled width from the first frame, overflowing offscreen-right into the space being reclaimed.
