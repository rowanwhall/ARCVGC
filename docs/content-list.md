...
| `Section(header, items, trailingAction?, centerHeader?)` | Grouping container with title and optional trailing action (e.g., `SectionAction.SeeMore` renders "See More" + chevron). Desktop-web-only: when `centerHeader = true` the title text is centered within its width instead of left-aligned (only honored when the section has no trailing controls; mobile + iOS ignore the flag). Used by the Pokemon-profile "Top Players" section. | `"section_{header}"` |
