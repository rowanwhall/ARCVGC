...
- **`AppConfigApi`** (declared in `shared/.../network/ApiService.kt`, alongside its implementer): Narrow interface (`suspend fun getConfig(): NetworkResult<AppConfig>`) that `ApiService` implements. `AppConfigRepository` depends on this interface (not `ApiService`) so unit tests can inject a non-networking fake — mirrors the `BattleRepositoryApi` testability pattern.
- **ApiService**: `getConfig(): NetworkResult<AppConfig>` — standard `NetworkResult` pattern; `override` of `AppConfigApi`
...
- **`AppConfigRepository`** (`shared/.../data/AppConfigRepository.kt`): Primary constructor takes `apiService: AppConfigApi`, `storage: AppConfigStorage`, `catalogCacheStorage: CatalogCacheStorage`, and `scope: CoroutineScope`. A **secondary 3-arg constructor** (without `scope`) delegates with `createSafeScope()` — this is an explicit secondary constructor rather than a default arg because Kotlin default parameter values don't bridge to Swift (a default would force every iOS call site to pass `scope`). All production callers (Android/iOS/Web) use the 3-arg form; tests call the 4-arg primary with a `TestScope` so the init cache-load + fetch runs deterministically under virtual time.
