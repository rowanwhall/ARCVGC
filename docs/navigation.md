...
- **Web (desktop)**: `BattleDetailPanel` renders inline in a right pane (`weight(0.6f)`) of a master-detail `Row` layout. List pane takes `weight(0.4f)`. Selected battle highlighted with `primaryContainer` background. No sheet/overlay. When the hosting page has its own left side pane (`SearchDesktopPage` filter pane, `UsageDesktopPage` rankings pane), that pane slides offscreen-left while a battle detail is open — the web `ContentListPage` reports open/close via `onBattleDetailOpenChanged` and the parent wraps its side pane in `CollapsibleSidePane`. See `docs/search.md` and `docs/content-list.md`.
