...
| `GET /api/v1/pokemon/{id}?format_id=N&lookback=W` | Pokemon profile (stats, top teammates/items/moves/abilities/tera types, top matches). `lookback` is `all`/`week`/`day`/`30days` (default `all`). |
