...
- **ContentListPage** — Generic paginated list page for Home (with logo + "ARC" branding in Orbitron font), Usage (Top Pokemon), Search results, per-Pokemon battles, Favorites sub-tabs. The Home page shows a `SectionGroup` of three weekly `StatChipRow` sections — "Weekly Top Pokémon", "Weekly Winners", and "Weekly Losers" (signed usage-change %) — fetched in parallel via `getFormatDetail`/`getPokemonUsage`/`getBestPreviousDay`, all on a 7-day window. Sort toggle in Search/Pokemon/Player modes. Desktop web also shows logo + "ARC" branding in the NavigationRail header. Home mode renders a gradient toolbar with a top-right "Submit replay" action (`AddLink` icon / `link.badge.plus` on iOS) that opens `SubmitReplayDialog` — a URL input that POSTs to `/api/v1/matches/` via `BattleRepository.submitReplay`.
